...
def get_dataset(dataset_name, data_path=None, subset="full", strict_version_check = False, **kwargs):
    ...
    if d.version_check is not None:
        ...
        if not passed:
            if strict_version_check:
                raise RuntimeError(f"Version check of the dataset '{dataset_name}' failed! Context[ {version_info} ] Aborting...")
            else:
                print(f"\033[1mWarning:\033[0m Version check of the dataset '{dataset_name}' failed! Context[ {version_info} ] Continuing nonetheless...")
